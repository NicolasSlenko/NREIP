function [thrust, torque, power] = bem(CLCD, pitch, CHORD, BETA, BLADE, v, rpm, rho, blades)
    ...
    function [DtDr, DqDr] = bemsolve(A, B)
      ...
      PHI=atan2(V0,V2);                                        % calculate angles
      CL = CLCD(1);
      CD = CLCD(2);
      % get lift and drag coefficients from our lift function
    ...
    options = optimoptions(@fminunc, 'MaxIterations', 100, 'SpecifyObjectiveGradient',true, "Display", "off");     % set options
    ABs = [A0(:); B0(:)];                                                       % pack A0, B0 into single column vector
    [ABs, ~] = fminunc(costfunction, ABs, options);                           % solving using unconstrained fmin     
    A0 = ABs(1:size(BLADE,2)); B0 = ABs((size(BLADE,2)+1):end);                 % unpack the A0 and B0 vectors
    A0 = A0'; B0 = B0';                                                         % put into row vector form
